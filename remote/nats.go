...
func NewNatsConfigManager(endpoints []string, keystore io.Reader) (crypt.ConfigManager, error) {
	store, err := NewNats(endpoints)
	if err != nil {
		return nil, err
	}
...
func NewStandardNatsConfigManager(endpoints []string) (crypt.ConfigManager, error) {
	store, err := NewNats(endpoints)
	if err != nil {
		return nil, err
	}
...
func NewNats(endpoints []string) (*Client, error) {
